...
const Editor = () => {
  ...
  // --- 2. UPDATED: HANDLE CALIBRATION FINISHED (Opens Modal) ---
  ...
  // --- 3. NEW: CONFIRM CALIBRATION (Called by Modal) ---
  ...
  // --- 4. NEW: UNLOCK SCALE (Called by Sidebar) ---
  ...
  return (
    <div className="flex h-screen bg-slate-900 font-sans overflow-hidden">
      ...
      <div className="absolute left-4 top-20 bottom-20 w-16 bg-slate-800 rounded-2xl border border-slate-700 shadow-2xl z-30 flex flex-col items-center py-4 gap-4">
        ...
        <button onClick={toggleJustification} className="w-10 h-10 rounded-xl flex flex-col items-center justify-center bg-slate-700 text-white border border-slate-600"><span className="text-xl">{justification === 'CENTER' ? '⌾' : justification === 'LEFT' ? '⇠' : '⇢'}</span></button>
        <button onClick={() => setOrtho(!ortho)} className={`w-10 h-10 rounded-xl flex flex-col items-center justify-center transition-all ${ortho ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-500'}`}><span className="text-xl">📐</span><span className="text-[6px]">ORTHO</span></button>
        <button onClick={() => setIsSnapping(!isSnapping)} className={`w-10 h-10 rounded-xl flex flex-col items-center justify-center transition-all ${isSnapping ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-500'}`}><span className="text-xl">🧲</span><span className="text-[6px]">SNAP</span></button>
      </div>
