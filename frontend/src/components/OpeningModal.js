...
const OpeningModal = ({ isOpen, openingData, scaleFactor, onConfirm, onCancel }) => {
    ...
    useEffect(() => {
        if (isOpen && openingData) {
            ...
            // Default the opening thickness to the wall's thickness, but let the user edit it
            setThickness(openingData.wallThickness || 0.23);
        }
    ...
    const handleSubmit = (e) => {
        ...
        onConfirm({
            ...
            finalThickness: parseFloat(thickness) // NEW: Pass thickness back to Editor
        });
    };
            ...
            <div className="bg-slate-800 rounded-2xl shadow-2xl border border-slate-700 w-96 overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    <div className="grid grid-cols-3 gap-3">
                        ...
                        <div className="space-y-2">
                            ...
                            <input 
                                ...
                                className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-3 text-white focus:border-yellow-500 focus:ring-1 outline-none transition-all"
                            />
                        </div>
                        {/* HEIGHT */}
                        <div className="space-y-2">
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <button type="submit" className="flex-1 px-4 py-3 rounded-xl font-bold text-xs uppercase tracking-wider text-slate-900 bg-yellow-500 hover:bg-yellow-400 transition-all shadow-[0_0_15px_rgba(234,179,8,0.3)]">Create</button>
                    </div>
                </form>
            </div>
        </div>
